...
import 'package:pigemshubshop/app/pages/product/add_product/widgets/image_file_preview.dart';
...
import 'package:pigemshubshop/app/widgets/pick_image_source.dart';
import 'package:pigemshubshop/core/domain/entities/product/product.dart';
import 'package:pigemshubshop/utils/compress_image.dart';

import 'package:pigemshubshop/utils/extension.dart';
import 'package:pigemshubshop/utils/numeric_text_formatter.dart';
                        ...
                        TextButton(
                          onPressed: () async {
                            if (_productImages.isNotEmpty &&
                                _productImages.length >= 5) return;
                  ...
                  onPressed: () async {
                    ...
                    if (_productImages.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                              'You must add at least 1 to 5 product images'),
                        ),
                      );
                      return;
                    }
  ...
  pushNotifyEndUsers() async {
    {
      notificationServices.getDeviceToken().then((value) async {
        var data = {
          ...
          'data': {'type': 'msj', 'id': 'Tarun Singh'}
        };
        // await http.post(Uri.parse('https://fcm.googleapis.com/fcm/send'),
        //     body: jsonEncode(data) ,
        //     headers: {
        //       'Content-Type': 'application/json; charset=UTF-8',
        //       'Authorization' : 'key=AAAXDFM:APyRKm6hrltV'
        //     }
        // ).then((value){
        //   if (kDebugMode) {
